...
const selected = ref<PlanDetail | null>(null)
...
const createForm = reactive<PlanForm>({
  name: '',
  status: 'active',
...
const editForm = reactive<PlanForm>({
  name: '',
  status: 'active',
  billing_type: 'one_time',
  event_credits: '',
...
const formatBillingType = (value?: string) => {
  if (!value || value === 'one_time') return 'Pago unico'
  if (value === 'subscription') return 'Suscripcion'
  return value
}
...
const getFeatureCount = (item?: PlanListItem | PlanDetail | null) => {
  if (!item || !Array.isArray(item.features)) return 0
...
const resetCreateForm = () => {
  ...
  createForm.billing_type = 'one_time'
  createForm.event_credits = ''
  createForm.credit_expiration_days = ''
  createForm.price_usd = ''
}

const hydrateEditForm = (item: PlanDetail) => {
  editForm.name = (item.name ?? '').toString()
  editForm.status = (item.status === 'inactive' ? 'inactive' : 'active') as 'active' | 'inactive'
  editForm.billing_type = (item.billing_type === 'subscription' ? 'subscription' : 'one_time') as 'one_time' | 'subscription'
  editForm.event_credits = item.event_credits === null || item.event_credits === undefined ? '' : String(item.event_credits)
  editForm.credit_expiration_days =
    item.credit_expiration_days === null || item.credit_expiration_days === undefined
      ? ''
      : String(item.credit_expiration_days)
  editForm.price_usd = item.price_usd === null || item.price_usd === undefined ? '' : String(item.price_usd)
}
...
const openPlan = async (item: PlanListItem) => {
  const id = item.id
  if (!id) return
  selectedId.value = id
  ...
  try {
    ...
    selected.value = data as PlanDetail
...
const submitCreate = async () => {
  if (!createForm.name.trim()) {
    notifyError('El nombre del plan es obligatorio.')
    return
  }
  ...
  try {
    const created = (await createPlan({
      ...
      event_credits: parseOptionalNumber(createForm.event_credits),
      credit_expiration_days: parseOptionalNumber(createForm.credit_expiration_days),
      price_usd: parseOptionalNumber(createForm.price_usd),
    })) as PlanDetail
...
const saveEdit = async () => {
  ...
  try {
    const updated = (await updatePlan(id, {
      ...
      event_credits: parseOptionalNumber(editForm.event_credits),
      credit_expiration_days: parseOptionalNumber(editForm.credit_expiration_days),
      price_usd: parseOptionalNumber(editForm.price_usd),
    })) as PlanDetail
...
const confirmDelete = async () => {
  ...
  try {
    await deletePlan(selected.value.id)
...
const applyStatusChange = async (id: string | number, nextStatus: 'active' | 'inactive') => {
  ...
  try {
    const data = (await updatePlanStatus(id, nextStatus)) as PlanDetail
...
watch(
  () => [filters.status, filters.billing_type],
  () => {
...
<template>
  <div class="bo-page container">
    <header class="bo-page-header">
      <h1>Planes</h1>
      <button class="primary-action" type="button" @click="openCreateModal">Crear plan</button>
    ...
    <section class="bo-card bo-filters-card">
      ...
      <div id="filters-panel" class="filters-panel" :class="{ open: isFiltersOpen }" role="region" aria-label="Filtros de planes">
        <div class="bo-filters">
          ...
          <div class="field field--billing">
            <label for="filter-billing">Facturacion</label>
            <select id="filter-billing" v-model="filters.billing_type">
    ...
    <section class="bo-content">
      <div class="bo-card bo-table">
        ...
        <div v-if="isLoading" class="bo-loading" role="status" aria-live="polite">Cargando planes...</div>

        <table v-else>
          <caption class="sr-only">Listado de planes</caption>
          <thead>
            <tr>
              ...
              <th scope="col" :aria-sort="getAriaSort('billing_type')"><button class="sort-button" type="button" @click="setSort('billing_type')"><span>Facturacion</span><span class="sort-indicator" :class="{ active: isSortActive('billing_type'), desc: isSortActive('billing_type') && sortDir === 'desc' }"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 9 6 6 6-6" /></svg></span></button></th>
              <th scope="col" :aria-sort="getAriaSort('event_credits')"><button class="sort-button" type="button" @click="setSort('event_credits')"><span>Creditos</span><span class="sort-indicator" :class="{ active: isSortActive('event_credits'), desc: isSortActive('event_credits') && sortDir === 'desc' }"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 9 6 6 6-6" /></svg></span></button></th>
              <th scope="col" :aria-sort="getAriaSort('price_usd')"><button class="sort-button" type="button" @click="setSort('price_usd')"><span>Precio</span><span class="sort-indicator" :class="{ active: isSortActive('price_usd'), desc: isSortActive('price_usd') && sortDir === 'desc' }"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 9 6 6 6-6" /></svg></span></button></th>
          ...
          <tbody>
            <tr
              ...
              tabindex="0"
              :aria-label="`Ver detalle de plan ${item.name ?? ''}`"
              ...
              <td>{{ formatBillingType(item.billing_type) }}</td>
              <td>{{ item.event_credits ?? '-' }}</td>
              <td>{{ formatPrice(item.price_usd) }}</td>
              <td v-if="showCreatedAt">{{ formatDate(item.created_at) }}</td>
            </tr>
            <tr v-if="!list.length">
              <td :colspan="showCreatedAt ? 7 : 6" class="bo-empty">No encontramos planes con los filtros actuales.</td>
            </tr>
          </tbody>
        </table>
      ...
      <aside class="bo-card bo-detail" :class="{ empty: !hasSelection }">
        ...
        <div v-else-if="selected" class="bo-detail-body">
          <div class="detail-row">
            ...
            <input v-else v-model="editForm.name" type="text" placeholder="Nombre del plan" />
          </div>
          ...
          <div class="detail-row">
            ...
            <strong v-if="!isEditing" class="dbl-edit-trigger" title="Doble clic para editar" @dblclick.stop="startEdit">{{ formatBillingType(selected.billing_type) }}</strong>
            <select v-else v-model="editForm.billing_type">
              <option value="one_time">Pago unico</option>
              <option value="subscription">Suscripcion</option>
            </select>
          </div>
          ...
          <div class="detail-row">
            <span>Creditos</span>
            <strong v-if="!isEditing" class="dbl-edit-trigger" title="Doble clic para editar" @dblclick.stop="startEdit">{{ selected.event_credits ?? '-' }}</strong>
            <input v-else v-model="editForm.event_credits" type="number" placeholder="Ej: 3" />
          </div>

          <div class="detail-row">
            <span>Expiracion de creditos (dias)</span>
            <strong v-if="!isEditing" class="dbl-edit-trigger" title="Doble clic para editar" @dblclick.stop="startEdit">{{ selected.credit_expiration_days ?? '-' }}</strong>
            <input v-else v-model="editForm.credit_expiration_days" type="number" placeholder="Ej: 365" />
          </div>

          <div class="detail-row">
            <span>Precio USD</span>
            <strong v-if="!isEditing" class="dbl-edit-trigger" title="Doble clic para editar" @dblclick.stop="startEdit">{{ formatPrice(selected.price_usd) }}</strong>
            <input v-else v-model="editForm.price_usd" type="number" step="0.01" placeholder="Ej: 19.99" />
          </div>

          <div class="detail-row">
            <span>Funcionalidades asociadas</span>
            <strong>{{ getFeatureCount(selected) }}</strong>
          </div>
    ...
    <div
      ...
      <div class="modal-card" @click.stop>
        ...
        <div class="modal-form">
          <label>
            <span>Nombre</span>
            <input v-model="createForm.name" type="text" placeholder="Ej: Premium" />
          </label>
          <label>
            <span>Estado</span>
          ...
          <label>
            <span>Creditos</span>
            <input v-model="createForm.event_credits" type="number" placeholder="Opcional" />
          </label>
          <label>
            <span>Expiracion de creditos (dias)</span>
            <input v-model="createForm.credit_expiration_days" type="number" placeholder="Opcional" />
          </label>
          <label>
            <span>Precio USD</span>
            <input v-model="createForm.price_usd" type="number" step="0.01" placeholder="Opcional" />
          </label>
        </div>
        <div class="modal-actions">
          ...
          <button class="primary-action" type="button" :disabled="isCreateLoading" @click="submitCreate">
            {{ isCreateLoading ? 'Creando...' : 'Crear plan' }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Confirmar eliminacion de plan"
      @click.self="cancelDelete">
      <div class="modal-card confirm-card" @click.stop>
        <h3>Eliminar plan</h3>
        <p>Vas a eliminar este plan de forma permanente. Esta accion no se puede deshacer.</p>
        <div class="modal-actions">
...
.field--billing { grid-column: 2; grid-row: 1; }
...
@media (max-width: 720px) {
  ...
  .bo-table table { min-width: 820px; }
...
@media (max-width: 650px) {
  ...
  .field--status { grid-column: 1; grid-row: 1; }
  .field--billing { grid-column: 1 / -1; grid-row: 2; }
