...
const selected = ref<SubscriptionDetail | null>(null)
...
type SortKey = 'id' | 'client' | 'plan' | 'status' | 'provider' | 'period_end' | 'created_at'
...
const formatRenewal = (value?: boolean | number) => {
  if (value === 1 || value === true) return 'Se cancela'
  return 'Renueva'
}
...
const openSubscription = async (item: SubscriptionListItem) => {
  const id = item.id
  if (!id) return
  selectedId.value = id
  ...
  try {
    ...
    selected.value = data as SubscriptionDetail
  } catch {
...
watch(
  () => [filters.client_id, filters.status, filters.provider, filters.date_from, filters.date_to],
  () => {
...
<template>
  <div class="bo-page container">
    ...
    <section class="bo-content" :class="{ 'is-detail-open': hasSelection }">
      <div class="bo-card bo-table">
        ...
        <div v-if="isLoading" class="bo-loading" role="status" aria-live="polite">Cargando suscripciones...</div>

        <table v-else>
          <caption class="sr-only">Listado de suscripciones</caption>
          <thead>
            <tr>
              ...
              <th scope="col" :aria-sort="getAriaSort('period_end')">
                <button class="sort-button" type="button" @click="setSort('period_end')">
                  <span>Fin de periodo</span>
                  <span class="sort-indicator" :class="{ active: isSortActive('period_end'), desc: isSortActive('period_end') && sortDir === 'desc' }">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                      <path d="m6 9 6 6 6-6" />
                    </svg>
                  </span>
          ...
          <tbody>
            <tr
              ...
              tabindex="0"
              :aria-label="`Ver detalle de suscripcion ${item.id ?? ''}`"
              :aria-selected="item.id === selectedId"
              @click="openSubscription(item)"
              @keydown.enter.prevent="openSubscription(item)"
              @keydown.space.prevent="openSubscription(item)">
              <td>{{ item.id ?? '-' }}</td>
              ...
              <td>{{ formatRenewal(item.cancel_at_period_end) }}</td>
              <td v-if="showCreatedAt">{{ formatDate(item.created_at) }}</td>
            </tr>
            <tr v-if="!list.length">
              <td :colspan="showCreatedAt ? 8 : 7" class="bo-empty">No encontramos suscripciones con los filtros actuales.</td>
            </tr>
          </tbody>
        </table>
  ...
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
    ...
    <path d="M6 6l12 12" />
        ...
        <div v-if="!hasSelection" class="bo-muted">Selecciona una suscripcion para ver el detalle.</div>
        ...
        <div v-else-if="selected" class="bo-detail-body">
          ...
          <div class="detail-row">
            ...
            <strong>{{ formatDate(selected.current_period_start) }}</strong>
          </div>
          <div class="detail-row">
            <span>Fin de periodo</span>
            <strong>{{ formatDate(selected.current_period_end) }}</strong>
          </div>
          <div class="detail-row">
            <span>Proveedor</span>
          ...
          <div class="detail-row">
            ...
            <strong>{{ formatRenewal(selected.cancel_at_period_end) }}</strong>
          </div>
          <div class="detail-row">
            <span>Cancelado</span>
            <strong>{{ formatDate(selected.canceled_at) }}</strong>
          </div>
          <div class="detail-row">
            <span>Creado</span>
